...
def stochastic_gradient_descent(X, y, w, epsilon, l2, rand_arr, update_lists = False, cost_func_vals=[], vals=[]):
    ...
    while repeat:
        ...
        gradient = second
        w_prev = w
        e1 = epsilon / iterations
        w = w - e1 * gradient
        if iterations < len(X) - 1:
            iterations += 1
            if update_lists:
...
def main():
    ...
    w = stochastic_gradient_descent(X, y, w, 1, 1, rand_arr, update_lists=True, cost_func_vals=cost_func_vals, vals=vals)
