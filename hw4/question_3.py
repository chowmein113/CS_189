...
def batch_gradient_descent(X, y, w, epsilon, l2, update_lists = False, cost_func_vals=[], vals=[]):
    ...
    while repeat:
        ...
        print(iterations)
        gradient = second + 2 * l2 * (w)
        w_prev = w
        w = w - epsilon * gradient
        if (cost_func_BGD(y, X, w, l2) > 0 or not np.array_equal(w_prev, w)) and iterations < 6000:
            iterations += 1
...
def main():
    ...
    w = batch_gradient_descent(X, y, w, 0.0006, 0.1, update_lists=True, cost_func_vals=cost_func_vals, vals=vals)
